...
WilcoxDETest <- function(
  ...
  if (limma.check[1]) {
    p_val <- my.sapply(
      X = 1:nrow(x = data.use),
      FUN = function(x) {
        return(min(2 * min(rankSumTestWithCorrelation_mod(index = j, statistics = data.use[x, ])), 1))
      }
    )
  } else {
...
rankSumTestWithCorrelation_mod <- function(index,statistics,correlation=0,df=Inf)
